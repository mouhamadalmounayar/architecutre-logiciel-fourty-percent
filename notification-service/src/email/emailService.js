...
class EmailService {
  ...
  async sendAlertNotification(recipient, title, message) {
    const text = `
${title}
...
<body>
  <div class="container">
    ...
    <div class="content">
      <div class="message-box"><p>${String(message || '').replace(/\n/g, '<br>')}</p></div>
    </div>
  ...
  async sendBatchAlerts(recipients, title, message) {
    const results = await Promise.allSettled(
      recipients.map(r => this.sendAlertNotification(r, title, message))
    );
